...
show(ProvidenceMap+
        geom_point(aes(x = as.numeric(as.character(Longitude)), y = as.numeric(as.character(Latitude))), data = alerts, color="red", size=1))

ProvidenceMap <- qmap("providence", zoom = 12, color = 'bw')

EPS1 <- 0.00452119
clusters <- dbscan(select(alerts, Longitude, Latitude), eps = EPS1, minPts = 2)
