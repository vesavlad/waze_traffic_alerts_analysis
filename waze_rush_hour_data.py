...
while True:
	...
	if os.path.isfile("waze_alerts_rush_hour_data.csv"):

		with open("waze_alerts_rush_hour_data.csv", "r", encoding="utf8") as f:
		    reader = csv.reader(f)
		...
		with open("waze_alerts_rush_hour_data.csv", "r", encoding="utf8") as f:
			...
			if len_reader>1:
				...
				if len_reader==1:
					...
					final_list1 = [['Country', 'Magvar', 'Sub type', 'City', 'Street', 'Report Rating', 'Confidence', 'Reliability', 'Latitude', 'Longitude', 'Type', 'Unique Alert ID', 'Publication Timestamp', 'Converted Publication Timestamp']]
	else:
		output1 = []
		final_list1 = [['Country', 'Magvar', 'Sub type', 'City', 'Street', 'Report Rating', 'Confidence', 'Reliability', 'Latitude', 'Longitude', 'Type', 'Unique Alert ID', 'Road Type', 'Publication Timestamp', 'Converted Publication Timestamp']]
	...
	for ele in content1:
		...
		if unique_alert_id not in output1:
			...
			publication_timestamp_converted = datetime.fromtimestamp(int(publication_timestamp)/1000).strftime('%Y-%m-%d %H:%M:%S')
			
			final_list1.append([country, magvar, sub_type, city, street, report_rating, confidence, reliability, longitude, latitude, Type, unique_alert_id, road_type, publication_timestamp, publication_timestamp_converted])
			print(final_list1)

			## Skip lines which are already present
			with open("waze_alerts_rush_hour_data.csv", "a", encoding='utf-8', newline='') as f:
				writer = csv.writer(f)
	...
	if os.path.isfile("waze_jams_rush_hour_data.csv"):

		with open("waze_jams_rush_hour_data.csv", "r", encoding="utf8") as f:
		    reader = csv.reader(f)
		...
		with open("waze_jams_rush_hour_data.csv", "r", encoding="utf8") as f:
			...
			if len_reader>1:
				...
				if len_reader==1:
					...
					final_list2 = [['Country', 'City', 'Level', 'Latitude', 'Longitude', 'Length', 'Turn type', 'Type', 'Unique Jam ID', 'End Node', 'Speed', 'Blocking Alert ID', 'Road type', 'Delay', 'Street', 'ID', 'Publication Timestamp', 'Converted Publication Timestamp']]
	...
	for ele in content2:
		...
		if unique_jam_id not in output2:
			...
			for loc in jam_line:
				...
				jam_publication_timestamp_converted = datetime.fromtimestamp(int(jam_publication_timestamp)/1000).strftime('%Y-%m-%d %H:%M:%S')
			
				final_list2.append([jam_country, jam_city, jam_level, jam_longitude, jam_latitude, jam_length, jam_turn_Type, jam_Type, unique_jam_id, jam_end_Node, jam_speed, jam_blocking_Alert_ID, jam_road_type, jam_delay, jam_street, jam_id, jam_publication_timestamp, jam_publication_timestamp_converted])
				print(final_list2)

				## Skip lines which are already present
				with open("waze_jams_rush_hour_data.csv", "a", encoding='utf-8', newline='') as f:
					writer = csv.writer(f)
